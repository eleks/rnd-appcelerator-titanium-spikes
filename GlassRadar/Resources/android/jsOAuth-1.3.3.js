...
exports.OAuth = function(global) {
    ...
    function stringToByteArray(str) {
        ...
        for (i = 0; str.length > i; i++) {
            ...
            128 > code ? bytes.push(code) : 2048 > code ? bytes.push(192 + (code >> 6), 128 + (63 & code)) : 65536 > code ? bytes.push(224 + (code >> 12), 128 + (63 & code >> 6), 128 + (63 & code)) : 2097152 > code && bytes.push(240 + (code >> 18), 128 + (63 & code >> 12), 128 + (63 & code >> 6), 128 + (63 & code));
        }
        return bytes;
    }
    function wordsToByteArray(words) {
        var i, bytes = [];
        for (i = 0; 32 * words.length > i; i += 8) bytes.push(255 & words[i >>> 5] >>> 24 - i % 32);
        return bytes;
    }
            ...
            this.request = function(options) {
                ...
                xhr.onreadystatechange = function() {
                    if (4 === xhr.readyState) {
                        ...
                        xhr.status >= 200 && 226 >= xhr.status || 304 == xhr.status || 0 === xhr.status ? success(responseObject) : xhr.status >= 400 && 0 !== xhr.status && failure(responseObject);
    ...
    OAuth.urlEncode = function(string) {
        ...
        for (i = 0; str_len > i; i++) if (c = string_arr[i].match(reserved_chars)) {
            c = c[0].charCodeAt(0);
            128 > c ? string_arr[i] = hex(c) : 2048 > c ? string_arr[i] = hex(192 + (c >> 6)) + hex(128 + (63 & c)) : 65536 > c ? string_arr[i] = hex(224 + (c >> 12)) + hex(128 + (63 & c >> 6)) + hex(128 + (63 & c)) : 2097152 > c && (string_arr[i] = hex(240 + (c >> 18)) + hex(128 + (63 & c >> 12)) + hex(128 + (63 & c >> 6)) + hex(128 + (63 & c)));
        }
    ...
    SHA1.prototype.hash = function(m) {
        ...
        ml = [ 255 & 8 * hb >> 24, 255 & 8 * hb >> 16, 255 & 8 * hb >> 8, 255 & 8 * hb, 255 & 8 * lb >> 24, 255 & 8 * lb >> 16, 255 & 8 * lb >> 8, 255 & 8 * lb ];
